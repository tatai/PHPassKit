...
use PHPassKit\Decorator\EventTicketArrayDecorator;
...
class EventTicketArrayDecoratorTest extends PHPUnit_Framework_TestCase {
	/**
	 * @var EventTicket
	 */
	...
	public function setup() {
		$this->_event_ticket = $this->getMock('PHPassKit\Style\EventTicket', array(), array(2));
		$this->_decorator_manager = $this->getMock('PHPassKit\Decorator\ArrayDecoratorManager');
		$this->_decorator = new EventTicketArrayDecorator($this->_decorator_manager);
	}
	...
	public function eachGroupOfFieldsIsUsed() {
		$this->_event_ticket->expects($this->exactly(5))->method('getKeys');

		$this->_decorator->decorate($this->_event_ticket);
	}	
	...
	public function whenHeaderFieldsIsNotSetThenItIsNotPresentInTheOutput() {
		$output = $this->_decorator->decorate($this->_event_ticket);
	...
	public function whenHeaderFieldsIsSetThenItIsDecorated() {
		...
		$this->_event_ticket->expects($this->any())->method('getKeys')->will($this->onConsecutiveCalls($headerFields));
	...
	public function whenPrimaryFieldsIsSetThenItIsDecorated() {
		...
		$this->_event_ticket->expects($this->any())->method('getKeys')->will($this->onConsecutiveCalls(null, $primaryFields));
	...
	public function whenSecondaryFieldsIsSetThenItIsDecorated() {
		...
		$this->_event_ticket->expects($this->any())->method('getKeys')->will($this->onConsecutiveCalls(null, null, $secondaryFields));
	...
	public function whenAuxiliaryFieldsIsSetThenItIsDecorated() {
		...
		$this->_event_ticket->expects($this->any())->method('getKeys')->will($this->onConsecutiveCalls(null, null, null, $auxiliaryFields));
	...
	public function whenBackFieldsIsSetThenItIsDecorated() {
		...
		$this->_event_ticket->expects($this->any())->method('getKeys')->will($this->onConsecutiveCalls(null, null, null, null, $backFields));
