...
class BoardingPassArrayDecorator {
	/**
	...
	public function decorate(BoardingPass $boardingPass) {
		$output = array();
		...
		$output['transitType'] = TransitType::getConstName($boardingPass->getTransitType());

		return $output;
	}

	private function _addField($name, &$output, BoardingPass $boardingPass) {
